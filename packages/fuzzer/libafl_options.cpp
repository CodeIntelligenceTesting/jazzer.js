...
LibAflOptions ParseLibAflOptions(Napi::Env env, const Napi::Object &js_opts) {
  ...
  const auto runs_set = js_opts.Get("runsSet");
  ...
  if (!runs.IsNumber() || !runs_set.IsBoolean() || !seed.IsNumber() ||
      !max_len.IsNumber() || !timeout_millis.IsNumber() ||
      !max_total_time_seconds.IsNumber() || !artifact_prefix.IsString() ||
      !corpus_directories.IsArray() || !dictionary_files.IsArray()) {
    throw Napi::Error::New(
        env, "The LibAFL backend expects an options object with mode, runs, "
             "runsSet, seed, maxLen, timeoutMillis, maxTotalTimeSeconds, "
  ...
  parsed.runs_set = runs_set.As<Napi::Boolean>().Value();
